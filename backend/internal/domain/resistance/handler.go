...
func (h *Handler) RegisterRoutes(r chi.Router) {
    ...
    func (h *Handler) ListExercises(w http.ResponseWriter, r *http.Request) {	exercises, err := h.repo.ListExercises(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusInternalServerError)
		return
	}
...
func (h *Handler) ListSessions(w http.ResponseWriter, r *http.Request) {
    ...
    // Or simpler: client fetches details?
    // Let's fetch inline for "History views"
...
func (h *Handler) CreateSession(w http.ResponseWriter, r *http.Request) {
    ...
    if err := json.NewDecoder(r.Body).Decode(&req); err != nil {
        // Default start time to now if missing?
        // Let's assume frontend sends it, or we handle it.
        // If decode fails, maybe empty body?
        // Let's default to Now if zero.
